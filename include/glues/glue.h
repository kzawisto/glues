...
template<typename T>
using Vector = boost::container::small_vector<T, boost::container::new_allocator<T>>;
...
template<typename ThisGlue, typename T>
struct MonadicTrait  : public ThisGlue{
	Data<T> data;
	using This = MonadicTrait<ThisGlue, T>;
	template<typename ...Args>
	MonadicTrait(const Data<T> & data, Args... args): data(data), ThisGlue(args...) {


	}
    template<typename Op2, typename K=ThisGlue>
    boost::optional<
    ReturnTypeBinary<Op2, typename K::output_type>> reduce( Op2 op2) {
...
template<typename T>
struct MonadicGlueTrivial{
	MonadicGlueTrivial (){

	}
    typedef T input_type;
    typedef T output_type;
    static std::vector<output_type> execute_monadic(input_type k) {
        return std::vector<output_type>{k};
    }
...
template<typename T>
MonadicTrait<MonadicGlueTrivial<typename std::remove_reference<decltype(*std::declval<T>())>::type>, T> mon(T begin, T end) {
    return MonadicTrait<MonadicGlueTrivial<typename std::remove_reference<decltype(*std::declval<T>())>::type>, T>(Data<T>{begin, end});
}

template<typename PrevGlue, typename Op>
struct MonadicGlueMap
 {
    ...
    PrevGlue prev;
    Op op;
	MonadicGlueMap(Op op, PrevGlue prev) :
			op(op), prev(prev) {

	}
std::vector<output_type> execute_monadic(input_type k) {
...
struct MonadicGlueFilter
 {
    ...
    PrevGlue prev;
    Op op;
	MonadicGlueFilter(Op op, PrevGlue prev) :
			op(op), prev(prev) {
...
template<typename PrevGlue, typename Op>
struct MonadicGlueFlatMap
 {
    typedef typename PrevGlue::input_type input_type;
    typedef typename PrevGlue::output_type this_input_type;
    typedef typename container_val<ReturnType<Op, this_input_type>>::type output_type;
    PrevGlue prev;
    Op op;
	MonadicGlueFlatMap(Op op, PrevGlue prev) :
			op(op), prev(prev) {
